...
function Slider() {
  ...
  return (
    ...
    scrollbar-none scroll-smooth overflow-clip sdff' ref={elementRef}> 
        {movieList.map((item)=>(
            <img src={IMAGE_BASE_URL+item.backdrop_path} key={item.id} 
            className='min-w-full  md:h-[310px] object-cover
            object-left-top mr-5 rounded-md hover:border-[4px]
            border-gray-400 transition-all duration-100 ease-in overflow-clip'/>
        ))}
    </div>
    </div>
  )
